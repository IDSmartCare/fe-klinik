...
const ModalAddPlan = ({
  ...
  const handleSave = () => {
    const modal: any = document.getElementById("add-plan");
    if (activeOption === "text") {
      ...
      ToastAlert({
        icon: "success",
        title: "Berhasil menambahkan Plan",
      });
      modal.close();
  ...
  useEffect(() => {
    async function getListSubjective() {
      const getRes = await fetch(
        `${process.env.NEXT_PUBLIC_URL_BE_KLINIK}/master-plan/${session?.user.idFasyankes}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    async function getAnswer() {
      const getRes = await fetch(
        `${process.env.NEXT_PUBLIC_URL_BE_KLINIK}/plan-answer`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${process.env.NEXT_PUBLIC_TOKEN}`,
  ...
  return (
    <dialog id="add-plan" className="modal">
      <div className="modal-box w-8/12 max-w-xl h-5/6 relative ">
        <AlertHeaderComponent message="Plan" />
        <div className="flex flex-col">
          ...
          {activeOption === "text" && (
            <>
              <div className="label">
                <span className="label-text">Tambah Plan</span>
              </div>
              <div className="h-80">
                <textarea
                  className="textarea textarea-bordered h-full w-full"
                  placeholder="Tambah Plan"
