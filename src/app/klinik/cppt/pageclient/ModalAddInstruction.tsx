...
const ModalAddInstruction = ({
  ...
  const [listInstruction, setListInstruction] = useState<any>([]);
  ...
  const handleSave = () => {
    const modal: any = document.getElementById("add-instruction");
    if (activeOption === "text") {
      ...
      ToastAlert({
        icon: "success",
        title: "Berhasil menambahkan Instruction",
      });
      modal.close();
  ...
  useEffect(() => {
    async function getListSubjective() {
      const getRes = await fetch(
        `${process.env.NEXT_PUBLIC_URL_BE_KLINIK}/master-instruction/${session?.user.idFasyankes}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    async function getAnswer() {
      const getRes = await fetch(
        `${process.env.NEXT_PUBLIC_URL_BE_KLINIK}/instruction-answer`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${process.env.NEXT_PUBLIC_TOKEN}`,
  ...
  return (
    <dialog id="add-instruction" className="modal">
      <div className="modal-box w-8/12 max-w-xl h-5/6 relative ">
        <AlertHeaderComponent message="Instruction" />
        <div className="flex flex-col">
          ...
          {activeOption === "text" && (
            <>
              <div className="label">
                <span className="label-text">Tambah Instruction</span>
              </div>
              <div className="h-80">
                <textarea
                  className="textarea textarea-bordered h-full w-full"
                  placeholder="Tambah Instruction"
