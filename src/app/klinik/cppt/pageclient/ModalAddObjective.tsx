...
const ModalAddObjective = ({
  ...
  const [listObjective, setListObjective] = useState<any>([]);
  ...
  const handleSave = () => {
    const modal: any = document.getElementById("add-objective");
  ...
  useEffect(() => {
    async function getListSubjective() {
      const getRes = await fetch(
        `${process.env.NEXT_PUBLIC_URL_BE_KLINIK}/master-objective/${session?.user.idFasyankes}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    async function getAnswer() {
      const getRes = await fetch(
        `${process.env.NEXT_PUBLIC_URL_BE_KLINIK}/objective-answer`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${process.env.NEXT_PUBLIC_TOKEN}`,
  ...
  return (
    <dialog id="add-objective" className="modal">
      <div className="modal-box w-8/12 max-w-xl h-5/6 relative ">
        <AlertHeaderComponent message="Objective" />
        <div className="flex flex-col">
          ...
          {activeOption === "text" && (
            <>
              <div className="label">
                <span className="label-text">Tambah Objective</span>
              </div>
              <div className="h-80">
                <textarea
                  className="textarea textarea-bordered h-full w-full"
                  placeholder="Tambah Objective"
